...
class CSVReader {
private:
    static string print_state(const std::ios &stream) {
        return "good()="
               + to_string(stream.good())
               + " eof()="
               + to_string(stream.eof())
               + " fail()="
               + to_string(stream.fail())
               + " bad()="
               + to_string(stream.bad());
    }
...
public:
     ...
     */
    static vector<vector<string>> csvToStringVecVec(const string &path, bool checkRowLength = false) {
        ...
        if (!arquivo.good())
            throw invalid_argument("Error reading file '" + path + "' -> " + print_state(arquivo));
